# Autoencoder Y  XGBoost Y  — merged from final4_1 (XGBoost working) + final4 (AE working)
...
class ProductionNIDSWorkspace:
    ...
    def build_console_ui(self):
        ...
        tab_xgb, tab_ae, tab_eda = st.tabs([
            "Supervised XGBoost Module",
            "Unsupervised Autoencoder Module",
            "Exploratory Data Analysis"
        ])

        # TAB 1: SUPERVISED XGBOOST MODULE
        with tab_xgb:
            xgb_metrics = st.container()
            st.markdown("---")
            xgb_charts = st.columns(2)
            st.markdown("---")
            xgb_logs = st.container()
            ...
            def draw_xgb_view():
                ...
                with xgb_logs:
                    ...
                    st.dataframe(st.session_state.visible_logs, use_container_width=True,
                                 hide_index=True, height=220)
                    if st.session_state.y_true_list and st.session_state.y_pred_list:
                        st.markdown("#### Performance Evaluation (Classification Report)")
                        st.dataframe(
                            pd.DataFrame(classification_report(
                                st.session_state.y_true_list, st.session_state.y_pred_list,
                                output_dict=True, zero_division=0
                            )).transpose().round(3), use_container_width=True)

            draw_xgb_view()

        # TAB 2: UNSUPERVISED AUTOENCODER MONITORING CONSOLE
        with tab_ae:
            st.markdown("### Autoencoder Parameter")
            param_c1, param_c2, _ = st.columns([2, 2, 2])

            with param_c1:
                # min=2 (KMeans floor), max=12 (CICIDS2017 ceiling)
                # Works correctly whether dataset has 4, 7, or 12 classes
            ...
            def draw_ae_view():
                ...
                with ae_logs:
                    st.markdown("#### Data History")
                    st.dataframe(st.session_state.ae_visible_logs, use_container_width=True,
                                 hide_index=True, height=220)
                    if st.session_state.ae_true_labels_history and st.session_state.ae_pred_labels_history:
                        st.markdown("#### DAE Classification Report")
        ...
        # TAB 3: GLOBAL EXPLORATORY DATA ANALYSIS (EDA) MODULE
        with tab_eda:
            ...
            if eda_dataset_type == "Training":
                ...
                sns.barplot(x=label_series.index, y=label_series.values, palette='flare', ax=ax1)
                ax1.set_yscale('log')
                ...
                ax2.pie([benign_cnt, attack_cnt], labels=['BENIGN', 'ATTACK'],
                        autopct='%1.1f%%', colors=['#4CAF50', '#F44336'],
                        startangle=90, textprops={'color': '#f0f6fc', 'fontsize':25})
                _buf2 = _io.BytesIO()
                fig2.savefig(_buf2, format='png', dpi=100, facecolor='#0b0f14')
                _buf2.seek(0)
                st.image(_buf2)
                plt.close(fig2)
                ...
                pcts_s = (label_series / total_train) * 100
                fig3, ax3 = plt.subplots(figsize=(8, 6), dpi=100)
                fig3.patch.set_facecolor('#0b0f14')
                ax3.set_facecolor('#0b0f14')
                colors3  = sns.color_palette('tab20', len(label_series))
                explode3 = [0.05] * len(label_series)
                wedges3, _ = ax3.pie(label_series, labels=None, startangle=140,
                                     colors=colors3, explode=explode3)
                legend_labels3 = [
                    f'{lbl} ({pct:.4f}%) [{cnt:,}]'
                    for lbl, pct, cnt in zip(label_series.index, pcts_s, label_series.values)
                ]
                ax3.legend(wedges3, legend_labels3, title="Traffic Classes",
                           loc="center left", bbox_to_anchor=(1.0, 0.5),
                           facecolor='#1c2128', edgecolor='#444c56',
                           labelcolor='#adbac7', title_fontsize=9, fontsize=8)
                plt.tight_layout()
                _buf3 = _io.BytesIO()
                fig3.savefig(_buf3, format='png', dpi=100, bbox_inches='tight', facecolor='#0b0f14')
                _buf3.seek(0)
                st.image(_buf3, use_container_width=True)
                plt.close(fig3)
                ...
                colors4 = sns.color_palette('crest', len(dur_series))
                ax4.barh(dur_series.index, dur_series.values, color=colors4)
                ax4.set_xscale('log')
        ...
        if st.session_state.stream_active and dataset_workspace is not None:
            if st.session_state.current_index < total_dataset_rows:
                ...
                # ── STEP D: VECTORIZED METRICS & LOG ACCUMULATION ──

                chunk_len = len(working_chunk)

                # --- Extract columns as pandas Series first so .str ops handle mixed types safely ---
                ...
                recon_scores    = chunk_errors  # already a numpy array from AE step

                # --- Boolean masks using pandas .str.upper() — safe for any residual non-string values ---
                is_attack_mask  = (actual_threats_s.str.upper() != 'BENIGN').values
                correct_mask    = (xgb_preds_arr == actual_threats)
                missed_mask     = (~correct_mask) & is_attack_mask & (xgb_preds_s.str.upper() == 'BENIGN').values
                ...
                # --- Flow duration & fwd packets (column lookup once, not per row) ---
                ...
                flow_dur_vals = working_chunk[flow_dur_cols[0]].fillna(0.0).values.tolist() \
                                if flow_dur_cols else [0.0] * chunk_len
                fwd_pkt_vals  = working_chunk[fwd_pkt_cols[0]].fillna(0.0).values.tolist() \
                                if fwd_pkt_cols else [0.0] * chunk_len

                # --- Extend time-series histories in one shot ---
                ...
                # --- XGBoost scalar counters (sum over boolean arrays) ---
                ...
                # --- Threat distribution (value_counts is fast pandas op) ---
                ...
                # --- Build log DataFrames directly (no per-row appends) ---
                ...
                ae_alert_labels = np.where(ae_alert_mask, "⚠️ Anamoly", "🟢 NORMAL")
